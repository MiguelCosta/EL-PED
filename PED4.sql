...
--
-- Host: localhost
-- Generation Time: Jan 06, 2012 at 03:55 PM
...
('bruno', 2, '2012-01-06 09:25:26'),
('miguel', 3, '2012-04-10 06:33:00');
...
--
-- Stand-in structure for view `ViewNumAccessDate`
--
CREATE TABLE IF NOT EXISTS `ViewNumAccessDate` (
`month` int(2)
,`year` int(4)
,`access` decimal(32,0)
);
...
--
-- Structure for view `ViewNumAccessDate`
--
DROP TABLE IF EXISTS `ViewNumAccessDate`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `ViewNumAccessDate` AS select month(`Access`.`datahora`) AS `month`,year(`Access`.`datahora`) AS `year`,sum(`Access`.`numAccess`) AS `access` from `Access` group by month(`Access`.`datahora`),year(`Access`.`datahora`) order by month(`Access`.`datahora`),year(`Access`.`datahora`);
